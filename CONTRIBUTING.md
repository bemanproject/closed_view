...
configure beman.closed_view to fetch them automatically via CMake FetchContent.
...
Project-specific options are prefixed with `BEMAN_CLOSED_VIEW`.
You can see the list of available options with:

```bash
cmake -LH -S . -B build | grep "BEMAN_CLOSED_VIEW" -C 2
```
...
### `BEMAN_CLOSED_VIEW_BUILD_TESTS`

Enable building tests and test infrastructure. Default: `ON`.
Values: `{ ON, OFF }`.

### `BEMAN_CLOSED_VIEW_BUILD_EXAMPLES`

Enable building examples. Default: `ON`. Values: `{ ON, OFF }`.

### `BEMAN_CLOSED_VIEW_INSTALL_CONFIG_FILE_PACKAGE`

Enable installing the CMake config file package. Default: `ON`.
Values: `{ ON, OFF }`.

This is required so that users of `beman.closed_view` can use
`find_package(beman.closed_view)` to locate the library.
